...
{% hint style="info" %}
BD681의 SW 버전이 1.2.0 이하일 경우, 스위치 설정을 알맞게 진행해야 합니다.
SW 버전이 1.3.0 이상일 경우, 별도의 스위치 설정은 필요 없습니다.
{% endhint %}
...
<mark style="color:green;">**- BD681 2개 구성 ( #1_BD681 + #2_BD681 + BD682 )**</mark>

![](../_assets/09_2.BD681_2개_케이블_연결.png)<br>
<그림 6. BD681 2개 케이블 연결>

위의 그림과 같이 #2 BD681 자리 옆에 #1 BD681을 꽂아 넣습니다.

{% hint style="info" %}
#1 BD681은 보드 스위치가 ON 되어야 합니다.
{% endhint %}

![](../_assets/11_2.BD681_2개_사용자DIO_보드_설정.png)<br>
...
사용자 DIO 보드를 2개 사용할 때, 사용자 DIO 보드 2개 모두 스위치가 OFF 되어있으면 <strong>"E55005 사용자 DIO 보드 모드 설정 오류 감지"</strong> 에러를 출력합니다.
해당 에러를 해결하기 위해서는 단독 구성인 사용자 DIO 보드 스위치를 ON으로 변경하여야 합니다.

{% hint style="warning" %}
"E55005 사용자 DIO 보드 모드 설정 오류 감지" 에러가 발생하였을 경우, 사용자 DIO 보드와 확장 DIO 보드를 정상적으로 사용할 수 없습니다. 보드의 스위치 설정을 올바르게 변경한 후에 사용해야 합니다.
{% endhint %}

{% hint style="info" %}
BD681의 SW 버전이 1.3.0 이상일 경우, 별도의 스위치 설정은 필요 없습니다.
{% endhint %}
...
{% hint style="info" %}
오픈 콜렉터 엔코더를 사용할 경우, '펄스 라인 에러' 기능은 사용할 수 없습니다.<br>
따라서 '입력 신호 할당'의 '펄스 라인 에러'를 '-1 (사용 안함)'로 설정해야 됩니다.
{% endhint %}

{% hint style="warning" %}
오픈 콜렉터 엔코더 사용할 경우, '펄스 라인 에러'를 '-1 (사용 안함)'로 설정하지 않으면 제어기 재부팅시 'E27001 (컨베이어 펄스 라인 이상)' 에러가 발생할 수 있습니다.
{% endhint %}
